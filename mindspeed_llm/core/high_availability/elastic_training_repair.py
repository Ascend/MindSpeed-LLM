...
def repair_callback(*repair_args, **repair_kwargs):
    ...
    if repair_type == RepairType.RT_SEND.value:
        tft_optimizer_data_repair.send_rank_repair(src_ranks, dest_ranks, optim_idxs, step, rank_list,
                                                   train_args[ha_constant.TRAIN_PARAM])
    elif repair_type == RepairType.RT_RECV_REPAIR.value:
        tft_optimizer_data_repair.recv_rank_repair(src_ranks, dest_ranks, optim_idxs, step, need_rebuild, rank_list,
                                                   train_args[ha_constant.TRAIN_PARAM])
    else:
