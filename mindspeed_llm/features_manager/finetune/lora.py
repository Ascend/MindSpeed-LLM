...
class LoraFeature(MindSpeedFeature):
    ...
    def register_patches(self, patch_manager, args):
        ...
        if is_enable_qlora(args):
            ...
            from mindspeed_llm.tasks.posttrain.lora.qlora import (
            ...
            patch_manager.register_patch(
                'megatron.core.tensor_parallel.layers.ColumnParallelLinear.__init__', parallel_linear_init_wrapper
            )
            patch_manager.register_patch(
                'megatron.core.tensor_parallel.layers.RowParallelLinear.__init__', parallel_linear_init_wrapper
            )
            patch_manager.register_patch(
                'megatron.core.tensor_parallel.layers.LinearWithFrozenWeight.forward', linear_with_frozen_weight_forward
            )
            patch_manager.register_patch(
            ...
            patch_manager.register_patch(
                'mindspeed.core.transformer.moe.grouped_gemm_util.Ops.gmm', grouped_gemm_util_ops_gmm
            )
            patch_manager.register_patch(
                'mindspeed.core.transformer.moe.moe_feature.overlap.moe_layer_overlap_all2all.gmm_op',
                moe_layer_overlap_all2all_gmm_op_wrapper,
            )
        else:
        ...
        from mindspeed_llm.tasks.posttrain.lora.qlora import dispatch_megatron_wrapper
