...
class FinetuneFeature(MindSpeedFeature):
    def __init__(self):
        super().__init__(feature_name="finetune", optimization_level=0)
    ...
    def register_args(self, parser: ArgumentParser):
        ...
        group.add_argument(
            '--no-cut-token', action='store_true', default=False, help='Used for not cut token in finetune.'
        )
        group.add_argument(
            '--full-shuffle-instruction-dataset', action='store_true', help='full shuffle instruction dataset or not'
        )
        ...
        group.add_argument('--no-pad-to-seq-lengths', action='store_true', help='Do not pad data to sequence lengths.')
    ...
    def pre_validate_args(self, args):
        ...
        if getattr(args, 'no_pad_to_seq_lengths', False):
            if args.log_throughput:
                ...
                warnings.warn(
                    "In no_pad_to_seq_lengths mode, accurate TFLOPS cannot be calculated, set --log-throughput to False.",
                    RuntimeWarning,
                )
    ...
    def post_validate_args(self, args):
        ...
        if (
            ...
            and getattr(args, 'reset_attention_mask', None)
