...
class MoERouter(MindSpeedFeature):
    def __init__(self):
        super().__init__(feature_name="moe_router", optimization_level=0)
    ...
    def register_args(self, parser):
        ...
        group.add_argument(
            '--router-gating-in-fp32', action='store_true', default=False, help='Compute router gating in float32.'
        )
        group.add_argument(
        ...
        group.add_argument("--topk-softmax-in-fp32", action='store_true', help="topk softmax in fp32.")
        group.add_argument(
            '--num-zero-experts', type=int, default=None, help='Number of Experts in MoE (None means no MoE)'
        )
    ...
    def validate_args(self, args):
        ...
        if args.n_hash_layers > 0:
            num_layers_per_pp_stage = args.num_layers // args.pipeline_model_parallel_size
            if args.n_hash_layers > num_layers_per_pp_stage:
                raise ValueError(
                    f'n_hash_layers ({args.n_hash_layers}) cannot exceed the number of layers in one PP stage '
                    f'({num_layers_per_pp_stage}).'
                )

            if (
                args.num_layers_per_virtual_pipeline_stage is not None
                and args.n_hash_layers > args.num_layers_per_virtual_pipeline_stage
            ):
                raise ValueError(
                    f'n_hash_layers ({args.n_hash_layers}) cannot exceed the number of layers in one VPP stage '
    ...
    def _validate_moe_args(self, args):
        ...
        if args.moe_expert_capacity_factor is not None:
            if args.moe_token_dispatcher_type == "allgather":  # nosec B105
                raise ValueError('moe_expert_capacity_factor not works with allgather token dispatcher')
            if args.moe_expert_capacity_factor < 0:
                args.moe_expert_capacity_factor = None
                print_rank0_by_args(  # pylint: disable=no-value-for-parameter
                    'When moe_expert_capacity_factor < 0, no token would be drop, so moe_expert_capacity_factor should be set to false.'
                )
            ...
            if args.shared_expert_gate_output_dimension not in (1, args.hidden_size):
    ...
    def _validate_group_limited_greedy(self, args):
        if args.moe_router_load_balancing_type == "group_limited_greedy":
            if args.moe_router_group_topk is None:
                ...
                raise AssertionError(
                    'The topk group ({}) should be less than n-group(EP)({}).'.format(
                        args.moe_router_group_topk, args.expert_model_parallel_size
                    )
                )
    ...
    def register_patches(self, patch_manager, args):
        ...
        patch_manager.register_patch('megatron.core.transformer.moe.router.TopKRouter.routing', topk_router_routing)
        patch_manager.register_patch(
            'megatron.core.transformer.moe.router.TopKRouter.forward', topk_router_forward_patch
        )
        patch_manager.register_patch('megatron.core.transformer.moe.router.TopKRouter.gating', topk_router_gating_func)
        patch_manager.register_patch('megatron.core.transformer.moe.router.z_loss_func', z_loss_func)
        if args.num_zero_experts:
            # add moe layer forward patch for longcat-flash-chat
            from mindspeed_llm.tasks.models.transformer.zero_experts_moe_layer import (
                zero_experts_moe_layer_init_wrapper,
                zero_experts_moe_forward,
            )

            patch_manager.register_patch(
                'megatron.core.transformer.moe.moe_layer.MoELayer.__init__', zero_experts_moe_layer_init_wrapper
            )
            patch_manager.register_patch(
                'megatron.core.transformer.moe.moe_layer.MoELayer.forward', zero_experts_moe_forward
            )

        elif not (args.moe_allgather_overlap_comm or args.moe_alltoall_overlap_comm):
            # add moe layer forward patch for deepseekv2
            patch_manager.register_patch('megatron.core.transformer.moe.moe_layer.MoELayer.forward', moe_layer_forward)
