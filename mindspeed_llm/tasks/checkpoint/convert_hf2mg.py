...
class Hf2MgConvert(Convert):
    ...
    def get_vpprank_hf_layeridxs(self) -> None:
        ...
        if self.mtp_num_layers:
            ...
            mtp_pp_rank = 0 if self.schedules_method == 'dualpipev' else self.pp_size - 1
    ...
    def load_matched_hf_weight(self, pp_rank, vpp_rank=None):
        ...
        if pp_rank == self.pp_size - 1 and self.schedules_method is None:
            st_filename_list.extend(list(layer_files_map_dict[hf_weight_key["final_layernorm"]]))
            st_filename_list.extend(list(layer_files_map_dict[hf_weight_key["output_layer"]]))
    ...
    def set_model_postprocess(self, hf_weight, mg_weight):
        ...
        for ep_rank in range(self.ep_size):
            ...
            for tp_rank in range(self.tp_size):
                ...
                if self.qlora_nf4:
                    self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, mg_weight_key["output_layer"],
    ...
    def set_mtp_preprocess(self, hf_layer_idx, mtp_layer_idx, hf_weight, mg_weight):
        ...
        for ep_rank in range(self.ep_size):
            ...
            for tp_rank in range(self.tp_size):
                ...
                if self.qlora_nf4:
                    self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, mg_weight_key["mtp_layers_eh_proj"],
    ...
    def set_model_layer_attn(self, hf_layer_idx, local_layer_idx, hf_weight, mg_weight, mtp_layer_flag=False):
        ...
        def _generate_mla_attn_layers_key(mtp_flag):
            if mtp_flag:
                ...
                q_b_key = mg_weight_key["mtp_layers_self_attention_linear_qb"]
                kv_b_key = mg_weight_key["mtp_layers_self_attention_linear_kvb"]
                ...
                q_b_key = mg_weight_key["layers_self_attention_linear_qb"]
                kv_b_key = mg_weight_key["layers_self_attention_linear_kvb"]
        ...
        if self.load_model.qkv_type == "pack_mla":
            ...
            q_b_proj = hf_weight.pop(hf_weight_key["layers_self_attention_linear_qb"])
            kv_b_proj = hf_weight.pop(hf_weight_key["layers_self_attention_linear_kvb"])
            ...
            if self.load_model.add_qkv_bias:
                ...
                qkv_bias_lst = torch.chunk(qkv_bias, self.tp_size, dim=0)
        ...
        for ep_rank in range(self.ep_size):
            for tp_rank in range(self.tp_size):
                if hasattr(self.load_model, "multi_head_latent_attention"):
                    ...
                    if self.mla_mm_split:
                        ...
                        if self.qlora_nf4:
                            self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, qk_nope_key, qk_nope_lst[tp_rank].clone())
                            self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, qk_rope_key, qk_rope_lst[tp_rank].clone())
                            self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, kv_nope_key, kv_nope_lst[tp_rank].clone())
                            self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, linear_v_key,
                        ...
                        if self.qlora_nf4:
                            self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, q_b_key, linear_qb_lst[tp_rank].clone())
        ...
        if hf_layer_idx >= first_k_dense_replace:
            ...
            for ep_rank in range(self.ep_size):
                ...
                for tp_rank in range(self.tp_size):
                    ...
                    if self.qlora_nf4:
                        self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, shared_fc1_key,
                                             shared_l0_lst[tp_rank].clone())
                        self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, shared_fc2_key,
            ...
            if self.moe_grouped_gemm:
                ...
                if self.moe_tp_extend_ep:
                    ...
                    gemm_fc1_ep = torch.chunk(
                        gemm_fc1.view(self.load_model.num_experts, self.load_model.hidden_size, -1), self.ep_size,
                        dim=0)
                    gemm_fc2_ep = torch.chunk(
                        gemm_fc2.view(self.load_model.num_experts, -1, self.load_model.hidden_size), self.ep_size,
                        dim=0)
                    ...
                    for tp_rank in range(self.tp_size):
                        if self.moe_tp_extend_ep:
                            mg_weight[ep_rank][tp_rank][experts_weight1_key] = gemm_fc1_ep[
                                ep_rank * self.tp_size + tp_rank].reshape(self.load_model.hidden_size, -1).clone()
                            mg_weight[ep_rank][tp_rank][experts_weight2_key] = gemm_fc2_ep[
                                ep_rank * self.tp_size + tp_rank].reshape(-1, self.load_model.hidden_size).clone()
                            if self.qlora_nf4:
                                self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, experts_weight1_key,
                                                     gemm_fc1_ep[ep_rank * self.tp_size + tp_rank].reshape(
                                                         self.load_model.hidden_size, -1).clone())
                                self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, experts_weight2_key,
                                                     gemm_fc2_ep[ep_rank * self.tp_size + tp_rank].reshape(-1,
                                                                                                           self.load_model.hidden_size).clone())
                        else:
                            ...
                            if self.qlora_nf4:
                                self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, experts_weight1_key,
                                                     gemm_fc1_ep_tp[tp_rank].reshape(self.load_model.hidden_size,
                                                                                     -1).clone())
                                self.qlora_nf4_quant(mg_weight, ep_rank, tp_rank, experts_weight2_key,
                                                     gemm_fc2_ep_tp[tp_rank].reshape(-1,
                                                                                     self.load_model.hidden_size).clone())
            else:
            ...
            for ep_rank in range(self.ep_size):
                ...
                for tp_rank in range(self.tp_size):
                    ...
                    if self.qlora_nf4:
                        self.qlora_nf4_quant(hf_weight, ep_rank, tp_rank,
                                             mg_weight_key["layers_mlp_linear_fc1"],
                                             mlp_l0_weight[tp_rank].clone())
                        self.qlora_nf4_quant(hf_weight, ep_rank, tp_rank,
                                             mg_weight_key["layers_mlp_linear_fc2"],
                                             mlp_l1_weight[tp_rank].clone())
            ...
            for pp_rank in range(self.pp_size):
                ...
                for ep_rank in range(self.ep_size):
                    for tp_rank in range(self.tp_size):
                        ...
                        torch.save({"model": mg_weight[ep_rank][tp_rank], "checkpoint_version": 3.0, "iteration": 1},
            ...
            for pp_rank in range(self.pp_size):
                ...
                for vpp_rank in range(self.vpp_size):
                    ...
                    if self.mtp_num_layers:
                        ...
                        norm_mtp_flag = self.schedules_method != 'dualpipev' and pp_rank == self.pp_size - 1 and vpp_rank == self.vpp_size - 1
                    ...
                    if self.schedules_method != 'dualpipev' and pp_rank == self.pp_size - 1 and vpp_rank == self.vpp_size - 1:
                        self.set_model_postprocess(hf_pp_weight, mg_weight[vpp_rank])
