...
在按照如下步骤操作前，需要先参考[MindSpeed LLM安装指导](../../install_guide.md)完成环境安装，并准备好模型权重和微调数据集。模型权重下载请参考[模型支持列表](../models/supported_models.md)文档中对应模型的下载链接。数据集下载请参考[Alpaca风格数据集](../solutions/finetune/datasets/alpaca_dataset.md)、[ShareGPT风格数据集](../solutions/finetune/datasets/sharegpt_dataset.md)和[Pairwise风格数据集](../solutions/finetune/datasets/pairwise_dataset.md)文档中数据集下载章节的内容。
