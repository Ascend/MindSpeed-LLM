...
数据集可通过 YAML 或 CLI 参数 `--data.dataset` 配置。示例脚本中统一放在 sh 脚本的 CLI 参数中，便于不同环境快速修改。
...
```bash
--data.dataset '{"file_name": "./my_data.json", "formatting": "alpaca"}'
```
...
# 常用参数通过 CLI 传入，CLI 参数优先级高于 YAML；也可按需移入 YAML
...
`dataset` 支持内联配置和通过 `dataset_info.json` 注册两种方式。推荐使用注册方式，以便进行多数据集混合训练；具体配置方法和示例请参考 [3.2 数据集配置](#32-数据集配置)。以上参数同样可通过 CLI 参数 `--data.dataset`、`--data.template` 等传入，CLI 优先级高于 YAML。
