...
FSDP2 后端可直接加载 HuggingFace 格式权重，开始训练前不需要执行模型权重转换。26.1.0 版本默认在对应的 `examples/fsdp2/**/*.yaml` 配置文件中，通过 `model.model_name_or_path` 配置模型权重目录。例如，Qwen3-8B 微调时需要修改 `examples/fsdp2/qwen3/tune_qwen3_8b_4k_fsdp2_A2.yaml` 中的以下参数：
...
26.1.0 版本默认在对应的 `examples/fsdp2/**/*.yaml` 配置文件中，通过 `data.dataset` 配置训练数据集。数据集参数支持内联配置和 `dataset_info.json` 注册名称两种写法。

以 Qwen3-8B 微调配置为例，默认使用 `dataset_info.json` 中注册的 `alpaca_full` 数据集：
...
内联配置适合快速验证本地数据集。修改对应 YAML 文件中的 `data.dataset`：
...
然后修改对应 YAML 文件中的 `data.dataset`，通过注册名称指定数据集：
...
模型权重路径、训练数据集、并行规模、batch size 和输出目录等参数均保存在配套 YAML 中。详细配置请参考 [Qwen3-8B 微调配置文件](../../../../../../examples/fsdp2/qwen3/tune_qwen3_8b_4k_fsdp2_A2.yaml)。YAML 示例内容如下：
...
打开对应的 `.sh` 文件，配置分布式训练参数。模型权重、数据集、并行规模和输出目录在配套 YAML 中配置。详细配置请参考 [Qwen3-8B 微调启动脚本](../../../../../../examples/fsdp2/qwen3/tune_qwen3_8b_4k_fsdp2_A2.sh)。
...
启动脚本通过以下命令读取配套 YAML，不需要在启动命令中重复配置训练参数：
...
多机训练时，需要在所有节点执行启动脚本，并分别设置对应的 `NODE_RANK`。训练日志默认保存在 `logs/` 目录，训练检查点保存在 YAML 中 `training.output_dir` 指定的目录。
