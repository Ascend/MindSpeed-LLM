...
详细参数见[性能数据采集](../tools/profiling.md)。官方工具的采集方式和适用场景请参考《性能问题通用定位指南》中的“[性能工具的使用](https://gitcode.com/Ascend/docs/blob/master/MindStudio/26.1.0/zh/cases/general_performance_issue_troubleshooting_guide/performance_tool_usage.md)”章节。
...
这些开关会增加采集开销和结果体积。采集结果可导入 [MindStudio Insight](https://gitcode.com/Ascend/msinsight/blob/master/docs/zh/user_guide/overview.md) 查看 Timeline、Operator、Communication 和 Memory。
...
上表列出了定位过程中最常用的 Timeline 泳道。每条泳道可展开查看具体细节，完整界面介绍参见 [MindStudio Insight 时间线](https://gitcode.com/Ascend/msinsight/blob/master/docs/zh/user_guide/system_tuning.md#%E6%97%B6%E9%97%B4%E7%BA%BFtimeline)。
...
1. 定位热点算子：在 [MindStudio Insight](https://gitcode.com/Ascend/msinsight/blob/master/docs/zh/user_guide/overview.md) 的 Operator 页签中按总耗时排序，结合调用次数、单次平均耗时、shape 和 dtype，识别总耗时较高或调用过于频繁的算子，分析是否可进一步优化计算逻辑。
...
在 TorchNPU 训练场景中，Host 侧（CPU）的算子调度、内存分配和任务下发与 Device 侧（NPU）的任务执行异步进行。当 Host 侧任务下发速度低于 Device 侧执行速度时，Device 会因等待新任务而进入空闲状态，形成 Host Bound。详细定位方法可参考[Host Bound问题定位及解决方法](https://gitcode.com/Ascend/docs/blob/master/MindStudio/26.1.0/zh/cases/general_performance_issue_troubleshooting_guide/solution_to_top3.md)。
