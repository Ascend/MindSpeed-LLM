...
# switch to the v2 framework
find . -type f -name "*.py" -exec sed -i 's/from mindspeed_llm import megatron_adaptor/from mindspeed_llm import megatron_adaptor_v2/g' {} +
sed -i 's/from mindspeed_llm.tasks import megatron_adaptor/from mindspeed_llm.tasks import megatron_adaptor_v2/g' mindspeed_llm/__init__.py
sed -i 's/parser = process_args(parser)/parser = process_args_v2(parser)/g' mindspeed_llm/training/arguments.py
