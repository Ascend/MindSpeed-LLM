...
def add_data_args(parser):
    ...
    group.add_argument('--handler-name', type=str, default="", help='specify a dataset handler')
    group.add_argument('--streaming', action='store_true', help='weather to use streaming')
    group.add_argument('--hf-datasets-params', default=None, help='huggingface load_dataset params')
    group.add_argument('--datasets', nargs='+', default=None, help='Paths to one or more input datasets to merge')
    group.add_argument(
    ...
    group.add_argument(
        ...
        help='Which template to use for constructing prompts in training.e.g., "qwen"',
    )
    group.add_argument(
        '--prompt-type-path', type=str, default=TEMPLATES_DIR, help='Path to the json file of templates.'
    )
    group.add_argument(
        '--dataset-additional-keys', nargs='*', default=[], help='Additional keys need to be add from dataset.'
    )
    group.add_argument(
        "--interleave-probs",
        default=None,
    ...
    group.add_argument("--dataset-dir", default=None, help="Path to the folder containing the datasets.")
    group.add_argument(
        "--overwrite-cache", action='store_true', help="Overwrite the cached training and evaluation sets."
    )
    group.add_argument(
        "--max-samples",
        type=int,
    ...
    group.add_argument("--seed", type=int, default=1234, help="Random seed to be used with data mix.")
    group.add_argument("--cache-dir", type=str, default="~/tmp", help="Where to store the cache of dataset from local.")
    group.add_argument("--map-keys", type=json.loads, default=None, help="Dataset field mapping.")
    group.add_argument(
        "--pack", action='store_true', help="Package multiple samples into one sample in a fine tuning dataset"
    )
    group.add_argument("--neat-pack", action='store_true', help="Use a zigzag attention mask.")
    group.add_argument("--script-data-dir", type=str, default=None, help="Python script dataset direction")
    group.add_argument(
        "--enable-thinking",
    ...
    group.add_argument("--pad-to-multiple-of", type=int, default=1, help="Pad each of the data to the multiple of...")
    group.add_argument("--data-obfuscation", action='store_true', help="Whether to enable data obfuscation.")
    group.add_argument("--obf-seed-content", type=str, default=None, help="Data obfuscation seed content.")
    group.add_argument(
        "--reasoning-effort",
        type=str,
        default=None,
...
def add_output_args(parser):
    ...
    group.add_argument(
        '--output-manifest', type=str, default=None, help='Optional path used to record generated index files'
    )
    ...
    group.add_argument('--n-subs', type=int, default=1, help='Number of subsets to cut for multiprocessing')
...
def validate_args(args):
    ...
    support_obfuscation_model = {"qwen3-32b": {"model_type": "qwen3", "hidden_size": 5120, "num_hidden_layers": 64}}
    ...
    if getattr(args, 'data_obfuscation', False):
        if not args.obf_seed_content or len(args.obf_seed_content) != 32:
            ...
            raise ValueError(
                f"When data obfuscation is enabled, the length of --obf-seed-content must be 32. Current length: {current_len}."
            )
        ...
        if args.tokenizer_name_or_path and os.path.exists(args.tokenizer_name_or_path):
            ...
            try:
                ...
                is_supported = any(
                    ...
                    and hidden_size == specs["hidden_size"]
                    and num_hidden_layers == specs["num_hidden_layers"]
                ...
                raise ValueError(
                    f"Failed to parse the configuration file. Please check the format of {config_file}."
                ) from e
        else:
            raise ValueError(
    ...
    if args.reasoning_effort and not args.enable_thinking:
        raise ValueError(
            f"reasoning_effort={args.reasoning_effort!r} requires --enable-thinking True. Either remove --reasoning-effort or enable thinking."
        )
...
def write_output_manifest(manifest_path, output_idx_files):
    """Atomically record the files generated by the selected dataset handler."""
    ...
    payload = {
        "idx_files": [output_idx_files[key] for key in sorted(output_idx_files)],
    }
...
def main():
    ...
    if args.n_subs == 1:
        ...
        handler.serialize_to_disk()
        output_idx_files = handler.output_idx_files
    else:
        ...
        for key in sub_idx_files[0].keys():
            ...
            output_idx_files[key] = target_idx

    write_output_manifest(args.output_manifest, output_idx_files)
